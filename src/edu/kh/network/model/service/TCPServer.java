...
public class TCPServer {
	...
	// -> 클라이언트 요청이 올 경우 이를 수락하고, 클라이언가 사용할 수 있는 소켓을 생성
	// -> 서버소켓과 클라이언트 소켓이 연결되어 데이터 통신이 가능해짐 
	
	
	public void serverStrart() {
	 ...
	 /* 사용할 변수 미리선언 */
		...
		BufferedReader br = null; // 입력용 보저 스트림 변수 
		...
		serverSocket = new ServerSocket(port); // 서버용 소켓을 생성하여 포트 결합	
			...
			// 서버용 소켓은 생성되고 나면 클라이언트 요청이 오기전까지 
			// 다음 코드 수행하지 않고 대기하고 있음
		System.out.println("[Server");	
		System.out.println("클라이언트 요청을 기다리고 있습니다.");	
		
		//4.접속요청이 오면 요청 수락후 해당 클라이언트에 대한 소켓 객체 생성
		...
		System.out.println(clientIP + "가 보낸 메세지 : "+ message);
			...
			try {
				...
				//  보조 스트림 close 시 연결딘 기반 스트림 (is,os)도 같이 close됨
				if(pw != null)pw.close();
				if(br != null)br.close();
				
				if(serverSocket != null) serverSocket.close();
				if(serverSocket != null) clientSocket.close();
